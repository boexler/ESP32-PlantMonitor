/**
 * @file pm_main.c
 * @brief Soil moisture sampler for XIAO ESP32-C6: WiFi → config → MQTT → deep sleep.
 */
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(pm_adc_read_averaged(CONFIG_PM_ADC_SAMPLE_COUNT, raw));
    ...
    ESP_ERROR_CHECK(pm_mqtt_publish_cycle(compact, &disc, moisture));
